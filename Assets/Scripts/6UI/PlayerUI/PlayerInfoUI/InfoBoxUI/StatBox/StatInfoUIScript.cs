...
public class StatInfoUIScript : MonoBehaviour
{
    ...
    private readonly int[] m_usingPoints = new int[(int)EStatInfoName.LAST];
    private int UsingPoint { get { int point = 0; for (int i = 0; i<(int)EStatInfoName.LAST; i++) { point += m_usingPoints[i]; } return point; } }
    ...
    {
        int[] noUse = new int[(int)EStatInfoName.LAST];
    ...
    public void SetStatNums(PlayerStatInfo _info, int[] _use)
    {
        ...
        {
            string num;
            float stat = _info.GetStat((EStatInfoName)i);
            if (_use[i] == 0)
            {
                num = stat.ToString();
            }
    ...
    public void ChkNUpgradeStat(EStatInfoName _stat, bool _up)
    {
        if (_up)
        {
    ...
    private void SetComps()
    {
        ...
        if(len != (int)EStatInfoName.LAST) { Debug.LogError("버튼 개수 다름"); return; }
        m_statNums = new TextMeshProUGUI[len];
        for(int i=0;i<len;i++) 
        {
            m_statNums[i] = m_statBtns[i].GetComponentInChildren<TextMeshProUGUI>();
            m_statBtns[i].SetComps(this, (EStatInfoName)i);
        }
