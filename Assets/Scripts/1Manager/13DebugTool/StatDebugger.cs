...
namespace Test
{
    ...
    public class StatDebugger : MonoBehaviour
    {
        ...
        private void Test()
        {
            ...
            Assert.IsTrue(Mathf.Approximately(stat.GetBonusValue("Test"), 10f), "Stat의 Bonus Value가 10이 아닙니다");
            Debug.Log($"Test Bonus Value: {stat.GetBonusValue("Test")}");

            Assert.IsTrue(stat.RemoveBonusValue("Test"), "Test Bonus Value의 삭제 실패");
            Assert.IsFalse(stat.ContainsBonusValue("Test"), "Test Bonus Value를 삭제하였으나 남아있습니다");
            ...
            Assert.IsFalse(stat.ContainsBonusValue("Test", "Test2"),"Test-Test2 Bonus Value를 삭제하였으나 남아있습니다");
