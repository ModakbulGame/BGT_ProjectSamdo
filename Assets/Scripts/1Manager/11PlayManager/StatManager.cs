...
public class StatManager : ObjectIdentified
{
    ...
    // key 값(object)은 bonus stat을 준 대상 (ex: 장비가 bonus Stat을 주었을 시 그 장비가 key값이 된다)
    // value Dictionary의 key 값은 Subkey
    // mainKey가 bonus stat을 여러번 줄 때 각 bonus 값을 구분하기 위한 용도
    // ex: 일정 스택에 도달하면 스탯을 주는 경우, mainKey=스택, subKey=스탯
    // subKey가 필요 없을 시, string.Empty를 subKey로 bonus 저장
