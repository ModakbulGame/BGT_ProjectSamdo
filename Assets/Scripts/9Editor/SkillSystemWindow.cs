...
public class SkillSystemWindow : EditorWindow
{
    ...
    // 현재 선택한 Data
    private static Dictionary<Type, ObjectIdentified> selectedObjectsByType = new();

    // Type별 Database(Category, Stat, Skill 등등...)
    private readonly Dictionary<Type, OIDatabase> databasesByType = new();
    ...
    private void SetupDatabases(Type[] dataTypes)
    {
        if (databasesByType.Count == 0)
        {
            ...
            {
                var database = AssetDatabase.LoadAssetAtPath<OIDatabase>($"Assets/Resources/Database/{type.Name}Database.asset");
                if (database == null)
                {
                    database = CreateInstance<OIDatabase>();
    ...
    {
        SetupStyle();
        SetupDatabases(new[] { typeof(Category), typeof(StatManager) });
    }
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.Width(300f));
            {
                ...
                if (GUILayout.Button($"New {dataType.Name}"))
                {
                    ...
                    var newData = CreateInstance(dataType) as ObjectIdentified;
